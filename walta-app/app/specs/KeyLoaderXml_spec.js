...
describe('KeyLoaderXml', function() {
	...
	it('should load a key from XML', function(){
		key = KeyLoaderXml.loadKey( 'specs/resources/simpleKey1/' );
		expect( key ).to.be.ok;
	});
	it('should have the correct toplevel node', function(){
		...
		expect( nd.questions[0].mediaUrls ).to.deep.equal(
			[ 'specs/resources/simpleKey1/media/couplet5p1.gif' ]
		);
		...
		expect( nd.questions[1].mediaUrls ).to.deep.equal(
			[ 'specs/resources/simpleKey1/media/couplet5p2.gif' ]
		);
	...
	it('should have the correct outcome for question root[1][1]', function(){
		...
		expect( nd.mediaUrls ).to.deep.equal(
			[ 'specs/resources/simpleKey1/media/parastacide_01.jpg',
			  'specs/resources/simpleKey1/media/parastacide_02.jpg' ]
		);
	});
	...
	it('should list the speed bug index with getSpeedbugIndex()', function() {
		...
		expect( sbug['maggots'].bugs ).to.deep.include.members( [{ imgUrl: 'specs/resources/simpleKey1/media/speedbug/athericidae.svg', refId: "athericidae" }] );
		expect( sbug['maggots'].bugs ).to.deep.include.members( [{ imgUrl: 'specs/resources/simpleKey1/media/speedbug/blepheraceridae.svg', refId: "blepheraceridae" }]  );

		expect( sbug['testNode2'].bugs ).to.be.ok;
		expect( sbug['testNode2'].bugs ).to.deep.include.members( [{ imgUrl: 'specs/resources/simpleKey1/media/speedbug/ranatra.svg', refId: "testNode2" }] );

		expect( sbug['larval'].bugs ).to.be.ok;
		expect( sbug['larval'].bugs ).to.deep.include.members( [{ imgUrl: 'specs/resources/simpleKey1/media/speedbug/hydrobiosidae.svg', refId: "dummyTaxon1" }] );
		expect( sbug['larval'].bugs ).to.deep.include.members( [{ imgUrl: 'specs/resources/simpleKey1/media/speedbug/megaloptera.svg', refId: "corydalidae" }] );
	});
	...
	it('should corectly find silohuettes from taxon nodes', function() {
		...
		expect( sbug.findSilhouette( taxon1 ) ).to.equal("specs/resources/simpleKey1/media/speedbug/ranatra.svg");
		expect( sbug.findSilhouette( taxon2 ) ).to.equal("specs/resources/simpleKey1/media/speedbug/hydrobiosidae.svg");
