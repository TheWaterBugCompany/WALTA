...
describe('KeyLoaderXml', function() {
	...
	it('should have the correct toplevel node', function(){
		...
		expect( nd.questions[0].mediaUrls ).to.deep.equal(
			[ 'spec/resources/simpleKey1/media/couplet5p1.gif' ]
		);
		...
		expect( nd.questions[1].mediaUrls ).to.deep.equal(
			[ 'spec/resources/simpleKey1/media/couplet5p2.gif' ]
		);
	...
	it('should list the speed bug index with getSpeedbugIndex()', function() {
		...
		expect( sbug['maggots'].bugs ).to.deep.include.members( [{ imgUrl: 'spec/resources/simpleKey1/media/speedbug/athericidae.svg', refId: "athericidae" }] );
		expect( sbug['maggots'].bugs ).to.deep.include.members( [{ imgUrl: 'spec/resources/simpleKey1/media/speedbug/blepheraceridae.svg', refId: "blepheraceridae" }]  );

		expect( sbug['testNode2'].bugs ).to.be.ok;
		expect( sbug['testNode2'].bugs ).to.deep.include.members( [{ imgUrl: 'spec/resources/simpleKey1/media/speedbug/ranatra.svg', refId: "testNode2" }] );

		expect( sbug['larval'].bugs ).to.be.ok;
		expect( sbug['larval'].bugs ).to.deep.include.members( [{ imgUrl: 'spec/resources/simpleKey1/media/speedbug/hydrobiosidae.svg', refId: "dummyTaxon1" }] );
		expect( sbug['larval'].bugs ).to.deep.include.members( [{ imgUrl: 'spec/resources/simpleKey1/media/speedbug/megaloptera.svg', refId: "corydalidae" }] );
	});
	...
	it('should corectly find silohuettes from taxon nodes', function() {
		...
		expect( sbug.findSilhouette( taxon1 ) ).to.equal("spec/resources/simpleKey1/media/speedbug/ranatra.svg");
		expect( sbug.findSilhouette( taxon2 ) ).to.equal("spec/resources/simpleKey1/media/speedbug/hydrobiosidae.svg");
