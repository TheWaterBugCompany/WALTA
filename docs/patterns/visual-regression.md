...
The devices CI covers are declared **once**, in [`visual/devices.json`](../../visual/devices.json). The `visual-devices` job publishes it, the two capture jobs build their matrices from it with `fromJSON`, and the report reads it to know which columns to expect. **Add a device there, not in the workflow** (then commit that leg's first CI-rendered baselines). Each leg uploads its captures/diffs as `visual-<platform>-<label>`.

### Every size band has a device in it

Layouts are written per band — `[if=Alloy.Globals.isHighRes]`, `isXHighRes`, `isShort`, `isSquare`. **A band with no device in the matrix is a set of rules nothing ever renders and no baseline ever covers**, so `visualDevices_spec.js` fails the build when one appears: the bands come from [`screenMetrics.BANDS`](../../walta-app/app/lib/util/screenMetrics.js), not from a list in the test, so adding a band to the app is adding a red test until a device renders it. The way out is a device **or** retiring the band — both are real answers; a band nothing reaches is dead code (a `isLowRes` band for screens under 300dp was retired that way: Android's smallest supported screen is 320dp and the smallest iPhone ever made is 320pt).

A device the matrix names may not exist on the runner as a device, only as a device *type* — the macOS image ships far more types than it creates devices for, and every iPhone it does create is 390dp or wider, which would leave the whole short band unrendered. So the iOS job creates the pinned device from its type when it is missing, rather than dropping the leg.

This is why each entry declares a `screen` — the landscape dp the bands are compared against. That figure is the device's to report, not ours to look up, so a capture run checks the declaration against what it measured and fails the run when they disagree (the report marks the column too, since the legs are advisory and their job status isn't read). Pick a device for the band's *risky end* rather than its middle, the way `small` covers `isShort`: the bug these catch is a fixed-dp layout overflowing the short end of a band that spans 300–700dp.

Coverage is per band, not per band per platform. `isSquare` has an iPad leg and no Android one because 4:3 is a shape iPads have and current Android tablets (16:10) don't — a leg added to tick the box would be testing a screen nobody owns, which is the failure this guard exists to prevent.
