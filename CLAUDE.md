...
Before writing or editing code, consult [docs/coding-style.md](docs/coding-style.md) — module system per directory, async/await direction, the defensive-code policy (trust inputs from systems we control; don't extract helpers for trivial logic; question cargo-culted defensiveness; let it error on unexpected input), the comment policy (short *why* comments are welcome; long, flowery ones are a refactor signal — and architectural narrative belongs in `docs/`, not source headers), and incremental migration guidance.
