(function(
	...
	if( true ){
		...
		if( true ){
			...
			// note: to get the file:// protocol to work in FF, you must set security.fileuri.strict_origin_policy to false in about:config
			 ...
			 false && has.add("dojo-force-activex-xhr",  true  && !doc.addEventListener && window.location.protocol == "file:");
			 true || has.add("native-xhr", typeof XMLHttpRequest != "undefined");
			if( true  && ! false ){
				getXhr = function(){
			...
			req.getXhr = getXhr;
			 ...
			 true || has.add("dojo-gettext-api", 1);
	...
	var	injectDependencies = function(module){
		...
		setArrived = function(module){
			...
			if(isEmpty(waiting)){
				clearTimer();
				 true  && legacyMode==xd && (legacyMode = sync);
			}
		},
		...
		getModule = function(mid, referenceModule, immediate){
			...
			if(match){
				...
				if( true  && legacyMode == sync && !plugin.executed){
					injectModule(plugin);
		...
		runFactory = function(module, args){
			...
			var factory = module.def,
				result;
			 true  && syncExecStack.unshift(module);
			...
			module.result = result===undefined && module.cjs ? module.cjs.exports : result;
			 true  && syncExecStack.shift(module);
		},
		...
		finishExec = function(module){
			...
			module.defOrder = defOrder++;
			 true  && forEach(module.provides, function(cb){ cb(); });
			if(module.loadQ){
	...
	if( true ){
		if( true ===undefined){
			 true || has.add("dojo-loader-eval-hint-url", 1);
		}
		...
		var fixupUrl= function(url){
				...
				if(has("config-dojo-loader-catches")){
					try{
						if(text===cached){
							...
							req.eval(text,  true  ? module.url : module.mid);
						}
					}catch(e){
			...
			defineModule = function(module, deps, def){
				...
				if( true  && legacyMode && !waiting[mid]){
	...
	if( true ){
		// the typically unnecessary !! in front of doc.attachEvent is due to an opera bug; see	#15096
		 undefined && has.add("ie-event-behavior", !!doc.attachEvent && (typeof opera === "undefined" || opera.toString() != "[object Opera]"));
	}
	...
	var def = function(
		...
		}else if(! undefined  || ! true  || injectingCachedModule){
...
var ret = {
	...
	withDoc: function(	/*DocumentElement*/ documentObject,
		...
		var oldDoc = ret.doc,
			oldQ =  false ,
			oldIE =  undefined , isIE, mode, pwin;

		try{
			dojo.doc = ret.doc = documentObject;
			// update dojo.isQuirks and the value of the has feature "quirks".
			// remove setting dojo.isQuirks and dojo.isIE for 2.0
			dojo.isQuirks =  false && has.add("quirks", dojo.doc.compatMode == "BackCompat", true, true); // no need to check for QuirksMode which was Opera 7 only

			if( undefined ){
				if((pwin = documentObject.parentWindow) && pwin.navigator){
					// re-run IE detection logic and update dojo.isIE /  undefined 
					...
					dojo.isIE =  undefined && has.add("ie", isIE, true, true);
				}
			}
			...
			dojo.isQuirks =  false && has.add("quirks", oldQ, true, true);
			dojo.isIE =  undefined && has.add("ie", oldIE, true, true);
		}
	}
};
...
define(["require", "module"], function(require, module){
	...
	if( true ){
		// Common application level tests
		 true || has.add("dom-addeventlistener", !!document.addEventListener);
		 true || has.add("touch", "ontouchstart" in document);
		...
		var form = document.createElement("form");
		 true || has.add("dom-attributes-explicit", form.attributes.length == 0); // W3C
		 true || has.add("dom-attributes-specified-flag", form.attributes.length > 0 && form.attributes.length < 40);	// IE8
	}
...
define(["../has", "./config", "require", "module"], function(has, config, require, module){
	...
	dojo.isAsync = ! true  || require.async;
	...
	// If  true  is truthy, then as a dojo module is defined it should push it's definitions
...
define(["./has"], function(has){
	...
	var hasJSON = typeof JSON != "undefined";
	 true || has.add("json-parse", hasJSON); // all the parsers work fine
		// Firefox 3.5/Gecko 1.9 fails to use replacer in stringify properly https://bugzilla.mozilla.org/show_bug.cgi?id=509184
	 true || has.add("json-stringify", hasJSON && JSON.stringify({a:0}, function(k,v){return v||1;}) == '{"a":1}');
	...
	if( true ){
		...
		return {
			parse:  true  ? JSON.parse : function(str, strict){
...
define(["./sniff", "./_base/window"],
	...
	// (currently known to work in all but IE < 10 and Opera)
	 "'WebkitUserSelect'" || has.add("css-user-select", function(global, doc, element){
	...
	if( true ){
		...
		if(! true ){
			...
			if(dua.indexOf("Gecko") >= 0 && !has("khtml") && ! true ){
				 undefined && has.add("mozilla", tv);
			}
			if( undefined ){
				//We really need to get away from this. Consider a sane isGecko approach for the future.
				 undefined && has.add("ff", parseFloat(dua.split("Firefox/")[1] || dua.split("Minefield/")[1]) || undefined);
			}
...
define(["./kernel", "./lang", "../sniff"], function(dojo, lang, has){
	...
	dojo.locale = dojo.locale || ( undefined  ? navigator.userLanguage : navigator.language).toLowerCase();
...
define(["./_base/kernel", "require", "./has", "./_base/array", "./_base/config", "./_base/lang", "./_base/xhr", "./json", "module"],
	...
	if(has("dojo-preload-i18n-Api") ||  true ){
...
define("dojo/request/xhr", [
	...
	'../_base/declare' =====*/
	 ...
	 true || has.add('native-xhr', function(){
		// if true, the environment has a native XHR implementation
		return typeof XMLHttpRequest !== 'undefined';
	});
	 false && has.add('dojo-force-activex-xhr', function(){
...
define(["require", "./_base/kernel", "./has"], function(aspect, dojo, has){
	...
	"use strict";
	if( true ){ // check to make sure we are in a browser, this module should work anywhere
		var major = window.ScriptEngineMajorVersion;
		has.add("jscript", major && (major() + ScriptEngineMinorVersion() / 10));
		has.add("event-orientationchange",  true  && !has("android")); // TODO: how do we detect this?
