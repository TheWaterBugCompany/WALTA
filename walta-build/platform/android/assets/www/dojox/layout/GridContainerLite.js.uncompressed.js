...
define("dojox/layout/GridContainerLite", [
		...
		[_LayoutWidget, _TemplatedMixin],
	{
		...
		postCreate: function(){
			...
			if(this.doLayout){
				this._border = {
					h:  undefined  ? geom.getBorderExtents(this.gridContainerTable).h : 0,
					w: ( undefined  == 6) ? 1 : 0
				}
			}else{
								...
								while(zone === null){
									if(node[pos] !== null && node[pos].nodeType !== 3){
										...
										if(pos === "previousSibling"){
											...
											node = node.parentNode.childNodes[ undefined  ? 0 : 1];
										}
									}
									zone = node[child];
