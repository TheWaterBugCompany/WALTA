...
- **A step intermittently fails right after a screen transition (tapped element "not present", or a tap seems to do nothing)** — a decision/detail window slides in over ~200ms; Appium taps far faster than a human, so a tap that lands mid-animation hits a stale coordinate and is silently dropped. **The fix is NEVER a fixed delay and NEVER a retry** (see "Flaky tests" below) — tap only once the element is *stationary*. Use `base-screen.js` `clickWhenStable(sel)` / `clickByTextWhenStable(text)`, which poll the element's own `getLocation()` via `driver.waitUntil` until two reads match, then click. `KeySearchScreen.choose` and `TaxonScreen.addToTrainingSample` are the reference call sites.

## Flaky tests: poll for state, never delay, never retry-to-go-green

Two rules, both non-negotiable (they are how real bugs get hidden):

1. **Never put a fixed delay in a test.** No `sleep(n)`, no `waitForTick(n)`, no "give it a moment". Always poll for the *actual state you expect* and proceed the instant it holds. The built-in waiters are the tools: `el.waitForDisplayed()` / `waitForExist()` / `waitForClickable()`, and `driver.waitUntil(async () => <predicate>, { timeout, timeoutMsg })` for anything custom (element stationary, element gone, a value present). Established precedents: `SummaryScreen.goDone` (waitUntil the screen is gone), `SiteDetailsScreen.waitForLocationLock` (waitUntil the lock text shows), `base-screen.clickWhenStable` (waitUntil the element stops moving). `wait-for-settled.js` (page-source stability) is for **screenshot** timing only — do not use it as a functional wait.
2. **Never add a retry to make a scenario pass.** Retries are reserved for genuinely contended *infrastructure* (a flaky device/session/network) — see `recover-session.js` / `dismiss-permission-alert.js`. A retry around functional steps hides broken code: if a single, correctly-awaited action doesn't produce the expected state, that is a bug to diagnose (in the app or in what you're waiting for), not to paper over. If you find yourself reaching for a retry to get green, stop and find the real state to poll for.

When a step flakes, the question is always "what settled state was I supposed to wait for, and didn't?" — not "how many times should I try?".
